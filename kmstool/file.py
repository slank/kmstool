...
class WorkContext(object):
    def __init__(self):
        ...
        self.payload = os.path.join(self.tmpdir, PAYLOAD_NAME)
...
def store_file(kms_client, key_id, input_path, output_path, context=None,
               chunksize=CHUNKSIZE):
    ...
    with WorkContext() as c:
        with open(c.enc_keyfile, 'w') as f:
            f.write(enc_key)
        encrypt_file(key, input_path, c.payload, chunksize)
        archive.add(c.enc_keyfile, arcname=ENCKEY_NAME)
        archive.add(c.payload, arcname=PAYLOAD_NAME)

    archive.close()


def retrieve_file(kms_client, input_path, output_path, context=None,
                  chunksize=CHUNKSIZE):
    ...
    with WorkContext() as c:
        ...
        decrypt_file(key, c.payload, output_path, chunksize)


def encrypt_file(key, input_path, output_path, chunksize=CHUNKSIZE):
    iv = ''.join(chr(random.randint(0, 0xFF)) for i in range(16))
    encryptor = AES.new(key, AES.MODE_CBC, iv)
    origsize = os.path.getsize(input_path)
...
def decrypt_file(key, input_path, output_path, chunksize=64 * 1024):
    with open(input_path, 'rb') as infile:
        iv = infile.read(16)
